        ...
        PseudoInfiniteViewport.Pannable
{
    ...
    public void paintComponent(Graphics g)
    {
        ...
        drawCheckeredBoard(g2);     
//         drawWatermark(g2);
   
        g2.setColor(Color.GREEN);
        drawCurrentLocation(g2); 
        if(GUIController.gameIsInSession())
        {
            Piece piece = (Piece) grid.get(currentLocation);
            if ( !(piece == null) && piece.getColor().equals(piece.getCurrentTurn()))
            {
                drawLocations(g2, piece.getValidMoveLocations());
            }  
            drawOccupants(g2); 
        }
        else
    ...
    private void drawCheckeredBoard(Graphics2D g2)
    {
        ...
        for( int row = 0; row < 8; row++)
        {
            if(row % 2 == 0)
            {
//                 firstColor = new Color(200, 180, 0);
//                 secondColor = new Color(180, 40, 0);
                firstColor = new Color(255, 206, 158);
                secondColor = new Color(209, 139, 71);
            } 
            else
            {
                firstColor = new Color(209, 139, 71);
                secondColor = new Color(255, 206, 158);
            }               
            for( int col = 0; col < 8; col+=2)
            {
                g2.setColor(firstColor);
                g2.fillRect(insets.top + col*(cellSize + 1), insets.left + row*(cellSize + 1), cellSize + 1, cellSize + 1);
                g2.setColor(secondColor);
                g2.fillRect(insets.top + (col + 1)*(cellSize + 1), insets.left + row*(cellSize + 1), cellSize + 1, cellSize + 1);
            }          
        }
    }
    ...
    private void drawLocations(Graphics2D g2, ArrayList<Location> locs)
    {
        if ("hide".equals(System.getProperty("info.gridworld.gui.selection")))
            return;
        for ( int i = 0; i < locs.size(); i++)
        {
            ...
            if (currentLoc != null)
            {
                Point p = pointForLocation(currentLoc);
                g2.fillRect(p.x - cellSize / 2 - 2, p.y - cellSize / 2 - 2,
                cellSize + 2, cellSize + 2);
                
                Graphics2D g3 = (Graphics2D) g2.create();
                g3.setColor(Color.RED);
                g3.drawRect(p.x - cellSize / 2 - 2, p.y - cellSize / 2 - 2,
                cellSize + 2, cellSize + 2);
...
//         g2.setPaint(new Color(0xE3, 0xD3, 0xD3));
        ...
        final int WATERMARK_FONT_SIZE = 65;
